...
public class GuardarCargar {
    ...
    public static void guardarLineas(List<String> lineas, String nombreArchivo, boolean append) throws IOException {
        File carpeta = new File(RUTA_DATOS);
        if (!carpeta.exists()) {
            carpeta.mkdirs();
        }
        ...
        File archivo = new File(RUTA_DATOS + nombreArchivo);
        try (PrintWriter writer = new PrintWriter(new BufferedWriter(new FileWriter(archivo, append)))) {
